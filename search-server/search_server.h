...
const int MAX_RESULT_DOCUMENT_COUNT = 5; //константы здесь?
...
public:
        ...
        : stop_words_(MakeUniqueNonEmptyStrings(stop_words))
    {
        if (any_of(stop_words_.begin(), stop_words_.end(), [](const std::string& word) { return !IsValidWord(word); })) {
            throw std::invalid_argument("Ќедопустимые символы в стоп-словах");
        }
    }
